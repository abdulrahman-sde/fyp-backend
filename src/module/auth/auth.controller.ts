...
import { registerSchema, loginSchema, onboardingSchema } from "./auth.validator.js";
...
export const onboardingHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await authService.completeOnboarding(userId, input);
  return ok(res, { user });
});
...
export const meHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await authService.buildAuthUser(full);
  return ok(res, { user });
});
